...
public class MyAlarmActivity extends AppCompatActivity {
    ...
    private int getIntervalFromRadioButtonId(int radioButtonId) {
        if (radioButtonId == R.id.interval5min) {
            return 5;
    ...
    private void startAlarm(int interval) {
        ...
        updateNotificationRunnable = new Runnable() {
            private int count = interval - 1; // interval 값에서 1을 빼줍니다.
            @Override
            public void run() {
                // 랜덤한 인용구 선택
                ...
                if (count > 0) {
                    ...
                    if (vibrator != null) {
                        vibrator.vibrate(6000); // 6초 동안 진동
                    }

                    // count 값을 다시 interval - 1로 설정
                    count = interval - 1;
        ...
        // 알람 시간 설정
        long alarmTime = System.currentTimeMillis() + (interval * 60 * 1000);

        // 알람 반복 설정
        alarmManager.setRepeating(AlarmManager.RTC_WAKEUP, alarmTime, 60 * 1000, pendingIntent);

        isAlarmSet = true;
        startButton.setEnabled(false);
        cancelButton.setEnabled(true);

        Toast.makeText(this, interval + "분마다 알람이 설정되었습니다.", Toast.LENGTH_SHORT).show();
    }
    ...
    private void showNotification(int interval) {
        ...
        // Add notification dismissal action
        Intent cancelIntent = new Intent(this, MyAlarmActivity.class);
        cancelIntent.setAction("CANCEL_ALARM_ACTION"); // Set action name
        PendingIntent cancelPendingIntent = PendingIntent.getActivity(this, 0, cancelIntent, PendingIntent.FLAG_IMMUTABLE);
        ...
        notificationBuilder.addAction(R.drawable.ic_cancel, "알람 해제", cancelPendingIntent);
